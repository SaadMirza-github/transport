...
<style>
    ...
    .badge-success-light {
        background-color: rgb(29 171 45 / 15%) !important;
        color: rgb(0 0 0);
    }
    ...
    <table id="example1" class="table table-striped table-bordered text-nowrap">
        ...
        @foreach($data as $val)
            <tr>
                ...
                <td><span class="badge badge-pill badge-default mt-0">{{get_user_name($val->user_id)}}<br></span></td>
                <td>{{$val->total_clicks}}</td>
                <td> <span class="badge badge-pill badge-success-light mt-0">{{get_pstatus($val->pstatus)}}</span></td>
                <td> <span class="badge badge-pill badge-warning-light mt-0">{{get_click_type($val->type)}}</span></td>
                <td> <span class="badge badge-pill badge-success mt-0">{{ get_pstatus(get_autoorder($val->order_id,'pstatus'))}}</span></td>
                <td>{{ count_history($val->user_id,$val->order_id)  }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
