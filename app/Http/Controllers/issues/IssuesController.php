...
class IssuesController extends Controller
{
    ...
    public function issue_comments_add($id)
    {
        ...
        //dd($chat);
        if (Auth::check()) {
            return view('main.issues.issues_comments_add', compact('issueid', 'chat'));
        } else {
            return redirect('/loginn/');
        }
    ...
    {
        $notificationdone = notifications::find($id);
    ...
    {
        $data['issues'] = issues::where('id', $id)->get();
        $data['users'] = User::all();
        $data['issuescomments'] = issue_chats::where('issueId', $id)->get();
        ...
        if (Auth::check()) {
            return view('main.issues.issues_view_admin', $data);
        } else {
            return redirect('/loginn/');
        }
